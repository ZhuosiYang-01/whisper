...
export function PaperApp() {
  ...
  async function initialize(supabase: SupabaseClient, token: string) {
    ...
    setContext(mine);
    await supabase.from("profiles").update({ timezone: Intl.DateTimeFormat().resolvedOptions().timeZone }).eq("id", session.user.id);
  ...
  async function claimUsername(name: string) {
    ...
    if (!supabase) {
      ...
      setContext({ username: name, space_id: null, partner_username: null, member_count: 0 });
      setPhase("space");
      return;
    }
  ...
  async function loginUsername(name: string) {
    ...
    if (!supabase) {
      ...
      setContext({ username: matched, space_id: null, partner_username: null, member_count: 0 });
      setPhase("space");
      return;
    }
    const credentials = await usernameCredentials(name);
...
function UsernameStep({ submit, login, error, clearError }: { submit: (name: string) => Promise<void>; login: (name: string) => Promise<void>; error: string; clearError: () => void }) {
  const [name, setName] = useState("");
  const [busy, setBusy] = useState(false);
  const [mode, setMode] = useState<"register" | "login">("register");
  async function onSubmit(event: FormEvent) { event.preventDefault(); setBusy(true); await (mode === "register" ? submit(name.trim()) : login(name.trim())); setBusy(false); }
  function switchMode() { setMode((value) => value === "register" ? "login" : "register"); setName(""); clearError(); }
  return <div className="flow-content"><h1>{mode === "register" ? "先给自己取个名字" : "欢迎回来"}</h1><form className="flow-form" onSubmit={onSubmit}><input id="username" name="username" aria-label="用户名" placeholder={mode === "login" ? "填写你的用户名" : undefined} value={name} onChange={(event) => setName(event.target.value)} minLength={2} maxLength={20} pattern="[A-Za-z0-9_\u4e00-\u9fa5]+" autoComplete="username" autoFocus required aria-describedby="username-hint username-error" />{mode === "register" && <small id="username-hint">2 到 20 个字，可用中文、字母、数字和下划线。</small>}{error && <p className="flow-error" id="username-error" role="alert">{error}</p>}<button className="flow-primary" disabled={busy || name.trim().length < 2}>{busy ? (mode === "register" ? "正在写下名字…" : "正在回来…") : (mode === "register" ? "就叫这个名字" : "登录")}</button><button className="account-switch" type="button" onClick={switchMode}>{mode === "register" ? "已有账号？点击登录" : "还没有账号？返回注册"}</button></form></div>;
}
